...
from db.queries.database import Base
...
updated_at = Annotated[datetime, mapped_column(
        ...
        server_default=text("CAST(DATE_TRUNC('day', TIMEZONE('utc-3', now())) AS DATE)"),
        # onupdate=datetime.now(tz=timezone(timedelta(hours=3.0))),
...
class Employees(Base):
    ...
    # reports: Mapped[List["Reports"]] = relationship(back_populates="employee", uselist=True)
...
class Places(Base):
    ...
    # reports: Mapped[List["Reports"]] = relationship(back_populates="place", uselist=True)
